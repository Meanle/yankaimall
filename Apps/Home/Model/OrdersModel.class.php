...
class OrdersModel extends BaseModel
{
    ...
    {
        $rd = array('status' => -1);          //设置默认状态码
        $USER = session('WST_USER');
        $goodsmodel = D('Home/Goods');      //实例化goodsmodel
        $morders = D('Home/Orders');        //实例化订单model
        $totalMoney = 0;                    //总金额
        $totalCnt = 0;                      //数量
        $userId = (int)session('WST_USER.userId');      //获取用户id
        ...
        $shopcart = $this->query($sql);                 //查询购物车

        $catgoods = array();                 //定义空数组catgoods
        $order = array();
        if (empty($shopcart)) {
            $rd['msg'] = '购物车内商品已生成订单，请去待付款看看';
            return $rd;
        } else {
            //整理及核对购物车数据
            ...
            for ($i = 0; $i < count($shopcart); $i++) {
                $cgoods = $shopcart[$i];
                $goodsId = (int)$cgoods["goodsId"];     //商品id
                $goodsAttrId = (int)$cgoods["goodsAttrId"];     //商品类型

                //if(in_array($goodsId, $paygoods)){      //if判断为false
                ...
                if ($goods['isNew'] == 1 || $goods['isRecomm'] == 1 || $goods['isBest'] == 1 || $goods['isHot'] == 1) {
                ...
                if ($goods['isNew'] == 1 || $goods['isRecomm'] == 1 || $goods['isBest'] == 1 || $goods['isHot'] == 1) {
                    ...
                    if (isValidVip() == 1) {
                        $goodsCatId3 = $goods['goodsCatId3'];
                        $sql = "select discount from __PREFIX__goods_cats where catId = $goodsCatId3";
                        $discount = $this->query($sql);
                        if ($discount) {
                            $discount = floatval($discount[0]['discount']);
                        } else {
                            $discount = 1;
                        }
...
//                $catgoods[$goods["shopId"]]["totalMoney"] = round(($catgoods[$goods["shopId"]]["totalMoney"] + ($goods["cnt"] * $goods["shopPrice"])),2);
            ...
            $rd['orderunique'] = $orderunique;
            $rd['isself'] = $isself;
            $rd['$needreceipt'] = $needreceipt;
            $rd['$payway'] = $payway;
            $rd['$consigneeId'] = $consigneeId;
            $rd['$userId'] = $userId;
            $rd['$catgoods'] = $catgoods;
    ...
    public function addOrders($userId, $consigneeId, $payway, $needreceipt, $catgoods, $orderunique, $isself, $rateMoney)
    {
        ...
        $addressInfo = UserAddressModel::getAddressDetails($consigneeId);   //地址信息
    ...
    public function queryReceiveByPage($obj)
    {
        ...
        $sql = "SELECT o.orderId,o.orderNo,o.shopId,o.orderStatus,o.userName,o.totalMoney,o.realTotalMoney,
		        o.createTime,o.payType,o.isRefund,o.isAppraises,sp.shopName, o.senderName, o.senderPhone
    ...
    public function getOrderDetails($obj)
    {
        ...
        $data['catName'] = $this->query("select gc.catName from wst_orders o, wst_order_goods og, wst_goods g, wst_goods_cats gc where o.orderId = og.orderId and og.goodsId = g.goodsId and g.goodscatid2 = gc.catid and o.orderId = $orderId");
    ...
    public function queryShopOrders($obj)
    {
        ...
        foreach ($data['root'] as $key => $v) {
            ...
            $cat = $data['root'][$key]['goodsCatId2'];
            $catName = $m->query("select catName from wst_goods_cats where catId = $cat");
            array_push($nameArr, $catName);
        }
        $data['catName'] = $nameArr;
    ...
    public function shopOrderDelivery($obj)
    {
        ...
        $sql = "UPDATE __PREFIX__orders set senderName = '" . $senderName . "', senderPhone = '" . $senderPhone . "', orderStatus = 3,deliveryTime='" . date('Y-m-d H:i:s') . "' WHERE orderFlag=1 and orderId = $orderId and shopId=" . $shopId;
        $rs = $this->execute($sql);
