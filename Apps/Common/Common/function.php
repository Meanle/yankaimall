...
function WSTDataFile($name, $path = '', $data = array())
{
    ...
    if (is_array($data) && !empty($data)) {
        if ($data['mallLicense'] == '') {
            if (stripos($data['mallTitle'], ' WSTMall') === false) $data['mallTitle'] = $data['mallTitle'] . " -  WSTMall";
        }
        $data = serialize($data);
 ...
 * @return bool
 */
function sms_old($tel)
{

    if (strlen($tel) != 11) return $tel;
    $code = authcode_length(4);
    $session = S('tel_code' . $tel);
    if ($session && NOW_TIME - $session['time'] < 24 * 60 * 60) {
        if ($session['sum'] >= 100) {
            return '-1';
        } else {
            $tel_code['sum'] = $session['sum'] + 1;
            $tel_code['time'] = $session['time'];
        }
    } else {
        $tel_code['sum'] = 1;
        $tel_code['time'] = NOW_TIME;
    }
//    if(NOW_TIME-)

    if(NOW_TIME - $session['time'] < 60){
        return '-1';
    }

    $tel_code['tel'] = authcode($tel);
    $tel_code['code'] = authcode($code);
    $tel_code['time'] = NOW_TIME;
    S('tel_code' . $tel, $tel_code);
    //短信接口
//    Vendor('sms.phpdemo');
//
//    $post_data = C('tel.svr_param');
//    $post_data['method'] = 'sendMsg';
//    $post_data['mobile'] = $tel;
//    $post_data['content'] = '@1@=,@2@=' . $code;
//    $post_data['msgtype'] = '2';             // 1-普通短信，2-模板短信
//    $post_data['tempid'] = 'JSM41401-0001'; // 模板编号
//    $post_data['code'] = 'utf-8';         // utf-8,gbk
//    $res = request_post(C('tel.svr_url'), $post_data);  // 如果账号开了免审，或者是做模板短信，将会按照规则正常发出，而不会进人工审核平台
    Vendor('AliSMS.aliyun-php-sdk-core.Config');
    $iClientProfile = DefaultProfile::getProfile("cn-hangzhou", "LTAIqD22g5LFjLyq", "1Y4LiU21l1HWaaUF7hoBo7st7IxaR3");
    $client = new DefaultAcsClient($iClientProfile);
    $request = new Sms\Request\V20160927\SingleSendSmsRequest();
    $request->setSignName("雁凯跨境馆");/*签名名称*/
    $request->setTemplateCode("SMS_25590316");/*模板code*/
    $request->setRecNum($tel);/*目标手机号*/
    $request->setParamString("{'code':'$code','product':'雁凯商城'}");/*模板变量，数字一定要转换为字符串*/
    try {
 ...
 * 判断验证码是否正确
 */
function true_sms($tel, $code)
{
    $tel_code = S('tel_code' . $tel);
    return (NOW_TIME - $tel_code['code_time'] < 60 && authcode($code) == $tel_code['code']);
}
 ...
 * 指定长度随机字符串
 */
function authcode_length($length)
{
    $code = '123456789';
    $new_code = '';
    for ($i = 0; $i < $length; $i++)
        $new_code .= $code[mt_rand(0, strlen($code) - 1)];
    return $new_code;
}
...
{
    $key = substr(md5('wxm'), 5, 8);
    $str = substr(md5($str), 8, 10);
    return md5($key . $str);
}

/***
 * @return int 0 表示会员卡已经过期了，-1 表示用户没有会员卡，1 表示会员卡有效
 */
function isValidVip()
{
    $card = M('vip_card')->where(array('userId' => session('WST_USER.userId')))->find();
    if (!$card) {
        return -1;//用户没有会员卡
    }
    if (strtotime($card['endTime']) < strtotime(date("y-m-d h:i:s"))) {
        return 0;//会员卡已过期
    } else {
        return 1;//会员卡有效
    }
