...
function Getaccess_tooken()
{

    $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".C('JX_WEIXIN.appid')."&secret=".C('JX_WEIXIN.secret');
    $JsonStr = file_get_contents($url);
    $contentArray = json_decode($JsonStr, true);
    return $contentArray['access_token'];
}

function Qiniufetch($UrlStr, &$QiuniuURL)
{
    $Qiniu = new \Think\Upload\Driver\Qiniu\QiniuStorage(C('QINIU_UPLOAD.SK'), C('QINIU_UPLOAD.AK'));
    $UrlStr = $Qiniu->Qiniu_Encode($UrlStr);
    $bucket = $Qiniu->Qiniu_Encode(C('QINIU_UPLOAD.BUCKET'));
    $token = $Qiniu->UploadToken(C('QINIU_UPLOAD.SK'), C('QINIU_UPLOAD.AK'));

    $ReturnStr = Qiniu_curl('http://iovip.qbox.me', array('fetch' => $UrlStr, 'to' => $bucket), $token);
    $QiuniuURL = $UrlStr . '---' . $bucket . '---' . $ReturnStr;
    return true;

}

function Qiniu_curl($url, $data, $token)
{
