...
//            $data["url"] = U('Home/WxPay/createQrcode', array("pkey" => base64_encode($pkey)));
            $data["url"] = U('Home/WxPay/createWxPay', array("pkey" => base64_encode($pkey)));
        }
        $this->ajaxReturn($data);
    }
    ...
    public function getBalanceURL()
    {
        ...
        if ($data["status"] == 1) {
            ...
            $data["url"] = U('Home/BalancePay/createBalancePay', array("pkey" => base64_encode($pkey)));
        }
        $this->ajaxReturn($data);
    }
    ...
    public function toPay()
    {
        ...
        $this->display('default/coffee/order_pay');
    }

    /**
     * 直接余额支付
     */
    public function directBalancePay()
    {
        $this->isUserLogin();
        $user = session('WST_USER');
        $morders = D('Home/Orders');
        ...
        $pm = D('Home/Payments');
        $payments = $pm->getList();
        $payments["onlines"][0]['payCode'] = 'Balance';
        $this->assign("payments", $payments["onlines"]);
