...
class GoodsAppraisesModel extends BaseModel
{
    ...
    public function getGoodsAppraises()
    {
        ...
        $sql = "SELECT ga.*, u.userName,u.loginName,u.userPhoto, od.createTime as ocreateTime 
    ...
    public function addGoodsAppraises($obj)
    {
        ...
        for ($i = 0; $i < count($goodsId); $i++) {
            //检测商品是否已评价
            $sql = 'select * from __PREFIX__goods_appraises where goodsId=' . $goodsId[$i] . ' and goodsAttrId=' . $goodsAttrId . ' and orderId=' . $orderId . ' and shopId=' . $shopId . ' and userId=' . $userId;
            $rs = $m->query($sql);
            if (!empty($rs)) {
                $rd['msg'] = '该商品已评价!';
                return $rd;
            ...
            $data["shopId"] = $shopId;
            $data["userId"] = $userId;
            $goodsScore[$i] = $goodsScore[$i] > 5 ? 5 : $goodsScore[$i];
            $goodsScore[$i] = $goodsScore[$i] < 1 ? 1 : $goodsScore[$i];
            $data["goodsScore"] = $goodsScore[$i];
            $data["timeScore"] = $timeScore;
            $data["serviceScore"] = $serviceScore;
            $data["content"] = I("content")[$i];
            $data['goodsAttrId'] = $goodsAttrId[$i];
            $data["isShow"] = 1;
