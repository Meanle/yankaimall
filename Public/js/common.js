	...
	try{
		...
		if(!notLimit){
			if(json.status && json.status=='-999'){
				var isDiv = $('#showImg').length;
				if(isDiv == 0) {
					var div = '<div id="showImg">' +
						'<p style="margin: 10px 0 5px 0;">请长按图片识别图中二维码</p>' +
						'<p>或微信搜索“雁凯跨境馆”并关注</p>' +
						'<p>若您已关注请退出公众号重新进入</p>' +
						'<img id="er-img" src="Apps/Home/View/default/images/er.png" />' +
						'<a href="javascript:;" id="close">我知道了</a>' +
						'</div>' +
						'<div id="warpper"></div>';
					$('body').append(div);
					$('#warpper').css({
					...
					$('#close').css({
						...
						location.href = "http://www.yankaimall.com";
					});
				}else {}
