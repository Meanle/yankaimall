...
class WxPayAction extends BaseAction {
	...
	public function createQrcode() {
		...
		if($pflag){
			$this->display ( "default/coffee/qrcode" );
		}else{
    ...
    public function createWxPay()
    {
        ...
        if (count($pkeys) != 3) {
            ...
            if ($needPay > 0) {
                ...
                $timeStamp = time();
                $out_trade_no = $timeStamp;
                $unifiedOrder->setParameter("openid",session('WST_USER.wxOpenId'));//商品描述
                ...
                $unifiedOrder->setParameter("notify_url",WEB_HOST.'/index.php/Home/WxPay/notifyJS');//通知地址
        ...
        if ($pflag) {
            ...
            $this->display("default/oders/pay_success");
        }
    }
