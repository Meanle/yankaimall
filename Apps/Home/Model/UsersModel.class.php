...
class UsersModel extends BaseModel {
	 ...
	 * 会员注册
	 */
    public function regist($autoRegister){
    	$rd = array('status'=>-1);
        $data = array();
        $loginName = '';
        if ($autoRegister){
            $data['loginName'] = $autoRegister['loginName'];
            $data['loginPwd'] = $autoRegister['loginPwd'];
            $data['reUserPwd'] = $autoRegister['reUserPwd'];
            $data['protocol'] = $autoRegister['protocol'];
            $data['userPhoto'] = $autoRegister['userPhoto'];
	    ...
	    $data['userType'] = 0;
        $data['wxOpenId'] = $autoRegister['wxOpenId'];
	    $data['userName'] = I('userName');
	    $data['userQQ'] = I('userQQ');
        if ($autoRegister['userPhoto']){
            $data['userPhoto'] = $autoRegister['userPhoto'];
        }else{
    ...
    public function getUserNum(){
        $cardId = I("cardId");
        $sql = "select userPhone from wst_vip_card left join wst_users ON wst_users.userId = wst_vip_card.userId  where cardId='".$cardId."'";
...
//		$userPhone = I("userPhone");
		...
		$userId = (int)$obj["userId"];
	    //检测账号是否存在
        $crs = $this->checkLoginKey($userId,false);
        if($crs['status']!=1){
	    	$rd['status'] = -2;
	    	return $rd;
	    }
...
//		$data["userPhone"] = $userPhone;
	    ...
	    if(false !== $rs){
            ...
            $WST_USER['userBirth'] = $data["userBirth"];
...
//			$WST_USER['userPhone'] = $data["userPhone"];
    ...
    public function changePhoneNum($tel,$id){
        $sql = "select * from __PREFIX__users where userId = ".$id;
        $row = $this->queryRow($sql);
        if ($row['userPhone'] == $tel){
            return 1;
        }
        $sql = "UPDATE wst_users SET userPhone = ".$tel." WHERE userId = ".$id;
        $rs = $this->execute($sql);
        if ($rs){
            return 1;
        }else{
     ...
     * 判断手机号是否绑定
     */
    public function phoneIsUse($tel){
        $sql = "select * from __PREFIX__users WHERE userPhone = ".$tel;
        $rs = $this->queryRow($sql);
        return $rs;
    }
    /**
     * 根据userId查找电话号码
     */
    public function phoneOfUser($userId){
        $sql = "select * from __PREFIX__users WHERE userId = ".$userId;
        $rs = $this->queryRow($sql);
        return $rs;
    }
